...
var Syncer = function (opts) {
  ...
  var emitFor = function (mapFnStr, changes) {
    var adjustedMap = function (doc, emits) {
      var map;
      var require; // eval should be run in a very limited context
  ...
  var syncMapDbFromSource = function (cb) {
    ...
    var changes = changes.results;
    var emitsByOrigin = emitFor(map.fn, changes);
    ...
    sourceToMapSeq = changes[changes.length - 1].seq;
    cb();
    });});});
  }
  ...
  var syncIncDbsFromMapDb = function (cb) {
    var syncsNum = 0;
    var stopped = false;
    for (incId in syncingIncsById) {
      syncsNum++;
      syncInc(incId, function (err) {
        syncsNum--;
        if (stopped) return;
        if (err) {
          stopped = true;
  ...
  var syncInc = function (incId, cb) {
    ...
    map.db.changes({since: inc.mapToIncSeq}, function (err, changes) {
    ...
    inc.mapToIncSeq = changes.last_seq;
