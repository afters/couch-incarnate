...
var Syncer = function (opts) {
  ...
  var verifyMapStringIsFunction = function () {
    ...
    if (fnStr.search("^function") === -1) throw new Error();
    fnStr = fnStr.substr("function".length);

    if (fnStr.charAt(0) !== '(') throw new Error();
    fnStr = fnStr.substr(1);

    var endOfArgs = fnStr.indexOf(")");
    if (endOfArgs === -1) throw new Error();

    var argsStr = fnStr.substring(0, endOfArgs);

    fnStr = fnStr.substr(endOfArgs+ 1);
    if (fnStr.charAt(0) !== '{') throw new Error();
    if (fnStr.charAt(fnStr.length - 1) !== '}') throw new Error();

    var bodyStr = fnStr.substring(1, fnStr.length - 1);
